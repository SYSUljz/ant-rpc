...
TEST(ProtobufTest, ServiceReflectionAndDynamicInvocation) {
  ...
  auto* req = dynamic_cast<EchoRequest*>(req_msg.get());
  ...
  auto* resp = dynamic_cast<EchoResponse*>(resp_msg.get());
...
TEST(ProtobufTest, FrameParserRejectsConfiguredMaxFrameBytes) {
  ...
  butil::IOBuf frame;
  PackRpcFrame(/*msg_type=*/0, /*correlation_id=*/7, "ant_rpc.EchoService", "Echo", request, frame);

  ASSERT_GT(frame.size(), sizeof(RpcHeader));
  const auto result = TryParseRpcFrame(frame, frame.size() - 1);
  EXPECT_EQ(result.status, FrameParseStatus::ERROR_FRAME_TOO_LARGE);
}
...
TEST(ProtobufTest, WireHeaderUsesNetworkByteOrderAndVersionOne) {
  ...
  butil::IOBuf frame;
  ASSERT_TRUE(PackRpcFrame(/*msg_type=*/0, /*correlation_id=*/7, "ant_rpc.EchoService", "Echo", request, frame));

  std::array<unsigned char, kRpcHeaderBytes> header {};
  frame.copy_to(header.data(), header.size(), 0);
  EXPECT_EQ(header[0], 'A');
  EXPECT_EQ(header[1], 'N');
  EXPECT_EQ(header[2], 'T');
  EXPECT_EQ(header[3], 'R');
  EXPECT_EQ(header[12], kRpcWireVersion);
  EXPECT_EQ(header[13], 0);
  EXPECT_EQ(header[14], 0);
  EXPECT_EQ(header[15], 0);

  const auto parsed = TryParseRpcFrame(frame);
  ASSERT_EQ(parsed.status, FrameParseStatus::SUCCESS);
  EXPECT_EQ(parsed.header.flags, MakeWireFlags());
  EchoRequest decoded;
...
TEST(ProtobufTest, RejectsInvalidMagicBeforeReadingDeclaredPayload) {
  ...
  EXPECT_EQ(TryParseRpcFrame(invalid).status, FrameParseStatus::ERROR_MAGIC_MISMATCH);
}
...
TEST(ProtobufTest, FrameLengthAndAttachmentBoundaryRoundTrip) {
  ...
  butil::IOBuf frame;
  ASSERT_TRUE(PackRpcFrame(meta, body, &attachment, frame));
...
TEST(ProtobufTest, RejectsUnsupportedWireVersionAndFeatureFlags) {
  ...
  butil::IOBuf valid;
  ASSERT_TRUE(PackRpcFrame(/*msg_type=*/0, /*correlation_id=*/7, "ant_rpc.EchoService", "Echo", request, valid));
  ...
  EXPECT_EQ(TryParseRpcFrame(unsupported_version).status, FrameParseStatus::ERROR_UNSUPPORTED_VERSION);

  bytes[12] = kRpcWireVersion;
  bytes[15] = 1;  // Lowest feature bit is unknown in V1.
  butil::IOBuf unsupported_flag;
  unsupported_flag.append(bytes.data(), bytes.size());
  EXPECT_EQ(TryParseRpcFrame(unsupported_flag).status, FrameParseStatus::ERROR_UNSUPPORTED_FLAGS);
}
...
TEST(ProtobufTest, RejectsAttachmentPastBodyAndDoesNotPackPastLimit) {
  ...
  EXPECT_EQ(TryParseRpcFrame(malformed).status, FrameParseStatus::ERROR_INVALID_LENGTH);

  EchoRequest request;
  request.set_message("will-not-fit");
  butil::IOBuf output;
  ASSERT_FALSE(PackRpcFrame(/*msg_type=*/0, /*correlation_id=*/7, "ant_rpc.EchoService", "Echo", request, output,
                            kRpcHeaderBytes));
...
TEST(ProtobufTest, RejectsOversizedDeclaredLengthsWithoutWaitingForPayload) {
  ...
  header[12] = kRpcWireVersion;
  butil::IOBuf malicious;
  malicious.append(header.data(), header.size());

  const auto parsed = TryParseRpcFrame(malicious);
  EXPECT_EQ(parsed.status, FrameParseStatus::ERROR_FRAME_TOO_LARGE);
  ...
  butil::IOBuf output;
  EXPECT_FALSE(PackRpcFrame(meta, body, nullptr, output, 20));
  EXPECT_TRUE(output.empty());
}
...
TEST(ProtobufTest, HandlesPartialAndBackToBackFrames) {
  ...
  butil::IOBuf first;
  butil::IOBuf second;
  ASSERT_TRUE(PackRpcFrame(/*msg_type=*/0, /*correlation_id=*/1, "ant_rpc.EchoService", "Echo", first_request, first));
  ASSERT_TRUE(
      PackRpcFrame(/*msg_type=*/0, /*correlation_id=*/2, "ant_rpc.EchoService", "Echo", second_request, second));
  ...
  const auto partial = TryParseRpcFrame(receive_buffer);
  EXPECT_EQ(partial.status, FrameParseStatus::NEED_MORE_DATA);
  ...
  const auto parsed_first = TryParseRpcFrame(receive_buffer);
  ASSERT_EQ(parsed_first.status, FrameParseStatus::SUCCESS);
  EXPECT_EQ(parsed_first.meta.correlation_id(), 1);
  receive_buffer.pop_front(parsed_first.total_frame_bytes);

  const auto parsed_second = TryParseRpcFrame(receive_buffer);
  ASSERT_EQ(parsed_second.status, FrameParseStatus::SUCCESS);
