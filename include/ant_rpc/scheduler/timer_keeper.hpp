...
class TimerKeeper {
 public:
  static constexpr size_t kNumBuckets = ant_rpc::constants::kDefaultTimerNumBuckets;
  static constexpr size_t kBucketMask = ant_rpc::constants::kDefaultTimerBucketMask;
  static constexpr size_t kHeapReserveSize = ant_rpc::constants::kDefaultTimerHeapReserveSize;
  ...
  class alignas(ant_rpc::constants::kCacheLineSize) Bucket {
   public:
 ...
 private:
  ...
  size_t get_bucket_index() const { return static_cast<size_t>(ant_rpc::utils::fast_random()) & kBucketMask; }
  ...
  alignas(ant_rpc::constants::kCacheLineSize) std::atomic<int64_t> global_nearest_run_time_us_;

  alignas(ant_rpc::constants::kCacheLineSize) std::atomic<uint64_t> next_seq_ {1};
  alignas(ant_rpc::constants::kCacheLineSize) std::atomic<bool> running_ {false};
  std::thread thread_;
};
