...
#include "ant_rpc/rpc/core/channel_state.hpp"
  ...
  if (was_running && log_failure) {
    ant_rpc::logging::Write(ant_rpc::logging::Event::kConnectionFailed, [&](auto& out) {
      out << " side=client connection_id=" << connection_id_ << " error_code=" << error_code
          << " reason=" << ant_rpc::logging::Quote(error_message);
    });
  }
...
#include "ant_rpc/rpc/io_driver/channel_connect.hpp"
#include "ant_rpc/rpc/io_driver/channel_receive.hpp"
#include "ant_rpc/rpc/io_driver/channel_write.hpp"
